...
pub fn spawn_tick_generator(remote: Remote, mut system_sender: SystemStreamRef) {
    ...
    let tick_interval = get_controller_tick_interval_millis();
    info!("Using tick interval of {} milliseconds", tick_interval);

    remote.spawn(move |handle| {
        // TODO: handle the failure to create the Interval
        let interval = Interval::new(Duration::from_millis(tick_interval), handle)
...
fn get_controller_tick_interval_millis() -> u64 {
