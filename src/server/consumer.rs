...
impl RotorConsumerNotifier {
    pub fn new(notifier: Notifier) -> RotorConsumerNotifier {
        RotorConsumerNotifier { notifier: notifier }
    }
}
...
pub fn get_last_event_id(url_path: &str) -> Result<EventId, String> {
    let query = url_path.find('?')
                        .map(|idx| url_path.split_at(idx + 1).1)
                        .unwrap_or(url_path);

    queryst::parse(query)
        .map_err(|_e| "wtf, mate?".to_string())
        .and_then(|params| get_last_event_id_from_params(&params))
}

fn get_last_event_id_from_params(params: &Value) -> Result<EventId, String> {
    params.find(PARAM_LAST_EVENT_ID)
          .map(|param_value| {
              param_value.as_string()
                         .unwrap()
                         .parse::<u64>()
...
pub fn init_consumer<S: EventStore>(request: Head,
    ...
    let consumer_id = scope.add_consumer_to_namespace(RotorConsumerNotifier::new(notifier),
                                                      last_event_id,
                                                      &namespace);
    ...
    Some((FloServer::Consumer(consumer_id), RecvMode::Buffered(1024), scope.now() + Duration::new(30, 0)))
}

pub fn on_wakeup<C, S>(consumer_id: usize, response: &mut Response, context: &mut FloContext<C, S>)
    where C: ConsumerNotifier,
          S: EventStore
{

    trace!("Waking up consumer: {}", consumer_id);
    context.get_next_event(consumer_id)
           .map(|event| {
...
mod test {
    ...
    fn get_last_event_id_returns_err_if_lastEvent_param_is_not_a_positive_integer() {
        assert_eq!("parameter \'lastEvent\' must be a positive integer, got: \"kdfk\"".to_string(),
                   get_last_event_id("/path?lastEvent=kdfk").unwrap_err());
        assert_eq!("parameter \'lastEvent\' must be a positive integer, got: \"-67\"".to_string(),
                   get_last_event_id("/path?lastEvent=-67").unwrap_err());
    }
