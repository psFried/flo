...
pub type SyncConnection = FloConnection<TcpStream>;

impl SyncConnection {
    pub fn connect<T: ToSocketAddrs>(addr: T) -> io::Result<SyncConnection> {
        TcpStream::connect(addr).and_then(|stream| {
            stream.set_read_timeout(Some(Duration::from_millis(1000))).map(|()| {
                SyncConnection::from_stream(stream)
            })
        })
    }

    pub fn from_stream(stream: TcpStream) -> SyncConnection {
        SyncConnection {
            writer: stream,
...
impl <T: Write + Read> FloConnection<T> {
    ...
    pub fn read(&mut self) -> io::Result<ServerMessage<OwnedFloEvent>> {
        let FloConnection{ref mut writer, ref mut read_buffer, ..} = *self;
