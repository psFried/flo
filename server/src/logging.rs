...
pub fn init_logging() {
    ...
    let root = Root::builder().appender(STD_OUT_APPENDER.to_string()).build(LogLevelFilter::Debug);
    ...
    let config = Config::builder().appender(appender).logger(rotor_logger).build(root).unwrap();
    init_config(config);
}
